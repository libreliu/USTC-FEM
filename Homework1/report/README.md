...
对于有限维空间 $ V_{h1} $，可以选择基函数 $ \{ \phi_i(x) \}_{i=1}^{n-1} $ 如下
$$
\phi_i(x) = \left\{
\begin{aligned}
...
> 之前一次实现中计算失误，将 $\int_0^1 \phi'_i(x) \phi'_j(x) dx$ 算成了 $ -1/(3h_{i+1}) $ ，应为 $ 1/(3h_{i+1}) $  ，并导致了实现上的错误。
>
> 正确的实现参见 $ V_{h2} $ 实现（续）。
...
修正了将 $\int_0^1 \phi'_i(x) \phi'_j(x) dx$ 算成了 $ -1/(3h_{i+1}) $ ，应为 $ 1/(3h_{i+1}) $  的问题后，刚度矩阵如下：

![image-20211001133151575](README.assets/image-20211001133151575.png)

然后，计算的结果如下：

![](README.assets/quad_working.png)

之前 $ n \ge 3 $ 就开始离谱的原因正是 $ \int \phi'_i \phi'_{i+1} dx $ 正式登场了...
...
| ---- | ------------- | ------ | ------------------ | ------ |
| 2   | 3.58E-04 | $ - $ | 7.86E-04 | $ -$ |
| 4 | 4.08E-05 | 3.13 | 1.12E-04 | 2.81 |
| 8 | 4.95E-06 | 3.04 | 1.48E-05 | 2.91 |
| 16 | 6.13E-07 | 3.01 | 1.91E-06 | 2.96 |
| 32 | 7.64E-08 | 3.00 | 2.41E-07 | 2.98 |
| 64 | 9.55E-09 | 3.00 | 3.02E-08 | 3.00 |
