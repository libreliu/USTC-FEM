...
$ F_i $ 处的数值积分采用取三角形三个边的中点的值平均，再乘上面积的方法来进行。
...
取 $ u = y(1-y)\sin(\pi x) $ ，则 $ f = (2+\pi^2(1-y)y)\sin(\pi x)$。

下面分别取了两个代表情况，即最粗和最细的网格的解的展示。

![image-20211123064124905](README.assets/image-20211123064124905.png)

![image-20211123064020710](README.assets/image-20211123064020710.png)

### 误差分析

在每个格点内部，用重心坐标选一些点后，计算误差得到下表。

> 此处取的是每个格点内部采样 16 个点。$ E_k $ 为三角形数目。
>
> 误差阶采用
> $$
> \text{ord} = 2 \frac{\ln \frac{err_k}{err_{k+1}}}{\ln \frac{E_{k+1}}{E_{k}}}
> $$
> 计算。

|   $E_k$   | $ L_\infty $     |  $ L_2$     | ord($L_\infty$) | ord($L_2$) |
| ---- | ---- | ---- | ---- | ---- |
|14|5.336e-02|7.399e-02|0.000|0.000|
|44|2.163e-02|2.489e-02|1.577|1.902|
|150|6.705e-03|7.416e-03|1.910|1.975|
|606|1.593e-03|1.924e-03|2.059|1.933|
|2396|3.697e-04|4.804e-04|2.125|2.019|

基本可以看出，收敛阶为 2。

同时，我们也可以均匀采样，得到误差的可视化图 $ | u(x, y) - u_h (x, y) | $ 如下

> 均匀取样 100 * 100 个点，下面分别为 gd0 ~ gd4 的结果

![](README.assets/Course_Uniform_Error.png)

![](README.assets/gd1_Uniform_Error.png)

![](README.assets/gd2_Uniform_Error.png)

![](README.assets/gd3_Uniform_Error.png)

![](README.assets/gd4_Uniform_Error.png)
